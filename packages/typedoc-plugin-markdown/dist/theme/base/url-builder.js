import { getFileNameWithExtension, isQuoted, removeFirstScopedDirectory, slugify, toPascalCase, } from '../../libs/utils/index.js';
import { OutputFileStrategy } from '../../options/maps.js';
import * as path from 'path';
import { DeclarationReflection, DocumentReflection, EntryPointStrategy, ReflectionKind, } from 'typedoc';
/**
...
export class UrlBuilder {
    ...
    constructor(theme, project) {
        ...
        this.entryFileName = getFileNameWithExtension(this.options.getValue('entryFileName'), this.fileExtension);
        this.isPackages =
    ...
    buildEntryUrls() {
        ...
        const isModulesOnly = this.project.children?.every((child) => child.kind === ReflectionKind.Module);
    ...
    buildUrlsFromPackage(projectChild) {
        ...
        let fullEntryFileName = getFileNameWithExtension(path.join(projectChild.name, packageEntryFileName), this.fileExtension);
        let fullIndexFileName = getFileNameWithExtension(path.join(projectChild.name, packagesIndex), this.fileExtension);
        ...
        const indexFileName = preservePackageReadme
            ? fullIndexFileName
            : fullEntryFileName;
        const isModulesOnly = projectChild.children?.every((child) => child.kind === ReflectionKind.Module);
        ...
        if (preservePackageReadme) {
            this.urls.push({
                url: useEntryModule
                    ...
                    : path.join(path.dirname(indexFileName), getFileNameWithExtension(packageEntryFileName, this.fileExtension)),
                model: projectChild,
                template: this.theme.readmeTemplate,
            });
            if (!useEntryModule) {
    ...
    buildUrlsForDocument(reflection) {
        ...
        if (mapping) {
            ...
            const directory = this.flattenOutputFiles
                ...
                : mapping.directory;
            const filename = [
                getFileNameWithExtension(reflection.name.replace(/ /g, '-'), this.fileExtension),
            ];
            if (reflection?.parent?.kind &&
                ![ReflectionKind.Module, ReflectionKind.Project].includes(reflection?.parent?.kind)) {
                filename.unshift(toPascalCase(ReflectionKind.singularString(reflection.parent?.kind)));
            }
    ...
    getUrl(reflection, urlPath, urlOptions) {
        ...
        if (this.options.getValue('outputFileStrategy') ===
            ...
            return urlPath.replace(getFileNameWithExtension('index', this.fileExtension), getFileNameWithExtension('module_index', this.fileExtension));
        }
        return urlPath;
    }
    ...
    getUrlPath(reflection, urlOption) {
        ...
        const filename = () => {
            if ([ReflectionKind.Module, ReflectionKind.Namespace].includes(reflection.kind) &&
                this.options.getValue('outputFileStrategy') ===
                    OutputFileStrategy.Modules &&
        ...
        return getFileNameWithExtension([parentDir, dir(), filename()].filter((part) => Boolean(part)).join('/'), this.fileExtension);
    }
    applyAnchorUrl(reflection, containerUrl) {
        ...
        if (anchorId) {
            ...
            if (this.options.getValue('outputFileStrategy') ===
                ...
                reflection.traverse((child) => {
                    this.applyAnchorUrl(child, containerUrl);
                });
            }
        }
    }
